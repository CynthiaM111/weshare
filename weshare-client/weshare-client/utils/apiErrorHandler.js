// Enhanced API Error Handler with comprehensive user messages
export const ERROR_CODES = {
    ...
    API_ENDPOINT_ERROR: 'API_ENDPOINT_ERROR', // New: for malformed/wrong URLs
...
const getSpecificErrorMessage = (error, errorCode) => {
    ...
    switch (errorCode) {
        case ERROR_CODES.INVALID_DATETIME:
            // Look for specific date/time issues
            ...
            // Default message with more specificity
        ...
        case ERROR_CODES.MISSING_FIELDS:
            ...
            // Check for multiple missing fields
        ...
        default:
            // If we have a specific server message, use it (but clean it up)
...
const isEndpointError = (error) => {
    ...
    // Check for common signs of malformed URLs
    const malformedIndicators = [
        url.includes('//'),           // Double slashes
        url.includes('undefined'),    // Undefined variables
        url.includes('null'),         // Null variables
        url.includes('[object'),      // Object serialization issues
        url.endsWith('/undefined'),   // Undefined parameters
        url.endsWith('/null'),        // Null parameters
        url.includes('/%'),           // Malformed encoding
        /\/\/+/.test(url),           // Multiple consecutive slashes
    ];
...
const isSearchOperation = (error) => {
    ...
    const searchIndicators = [
        ...
        url.includes('query='),
        url.includes('filter='),
        error?.config?.params // Has query parameters
    ];
...
export const isRetryableError = (error) => {
    const retryableCodes = [
        ...
        // Note: API_ENDPOINT_ERROR is NOT retryable since it's a config issue
    ];
